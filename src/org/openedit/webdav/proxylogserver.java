...
public class proxylogserver extends proxyserver
{
	...
	public proxylogserver(String _thost, int _port, int inTar)
	{
		super(_thost, _port);
		tport = inTar;
	}
	...
	public static void main(String args[])
	{
		...
		if( args.length > 2)
		{
			Integer target = new Integer(args[2]);
			proxylogserver us = new proxylogserver(args[0], iport.intValue(), target.intValue());
			us.go();
			
		}
		else
		{
			proxylogserver us = new proxylogserver(args[0], iport.intValue(), iport.intValue());
			us.go();
...
class proxyserver implements Runnable
{
	...
	protected int tport, lport; // tport is port on target host, lport is listen port
	...
	public void run()
	{
		...
		{
			if (debug)
				System.err.println("proxyserver: " + lport + " " + T.toString());
			T.printStackTrace();
		}
	}
...
class proxylogconn extends proxyconn
{
	...
	protected void log(boolean fromc1, byte[] d)
	{
		try
		{
			...
			String ss = printableBytes(d);
			if( ss.contains("If: <http://localhost:7777/stuff/litmus/lockme> (<opaquelocktoken:/stuff/litmus/lockme>)"))
			{
				System.out.println("Put a break point. In PUT or IFHANDLER");
			}
